...
export class Area extends XYPath {
	...
	private val0Fn() {
		return this.value0Property ? (val?):number => {return this.valueScale.map(val[this.value0Property])} : (val?):number => { return this.valueScale.map(0)}
	}
	...
	protected pathGenerator = new AreaGenerator(this.spline, this.keyFn, this.valFn, this.val0Fn(), this.isVertical)
}